...
void SCommonImGuiWidget::Construct(const FArguments& InArgs)
{
	...
	ModuleManager->OnPostImGuiUpdate().AddRaw(this, &SCommonImGuiWidget::OnPostImGuiUpdate);
	...
	ChildSlot
	[
		SAssignNew(CanvasControlWidget, SImGuiCanvasControl).OnTransformChanged(this, &SCommonImGuiWidget::SetImGuiTransform)
	];
...
SCommonImGuiWidget::~SCommonImGuiWidget()
{
...
void SCommonImGuiWidget::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	UpdateCanvasSize(AllottedGeometry);
}

FReply SCommonImGuiWidget::OnKeyChar(const FGeometry& MyGeometry, const FCharacterEvent& CharacterEvent)
{
	return InputHandler->OnKeyChar(CharacterEvent);
}

FReply SCommonImGuiWidget::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& KeyEvent)
{
	UpdateCanvasControlMode(KeyEvent);
	return InputHandler->OnKeyDown(KeyEvent);
}

FReply SCommonImGuiWidget::OnKeyUp(const FGeometry& MyGeometry, const FKeyEvent& KeyEvent)
{
	UpdateCanvasControlMode(KeyEvent);
	return InputHandler->OnKeyUp(KeyEvent);
}

FReply SCommonImGuiWidget::OnAnalogValueChanged(const FGeometry& MyGeometry, const FAnalogInputEvent& AnalogInputEvent)
{
	return InputHandler->OnAnalogValueChanged(AnalogInputEvent);
}

FReply SCommonImGuiWidget::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return InputHandler->OnMouseButtonDown(MouseEvent).LockMouseToWidget(SharedThis(this));
}

FReply SCommonImGuiWidget::OnMouseButtonDoubleClick(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SCommonImGuiWidget::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	FReply Reply = InputHandler->OnMouseButtonUp(MouseEvent);
	if (!ImGuiWidgetUtils::NeedMouseLock(MouseEvent))
	{
...
FReply SCommonImGuiWidget::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return InputHandler->OnMouseWheel(MouseEvent);
}

FReply SCommonImGuiWidget::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return InputHandler->OnMouseMove(TransformScreenPointToImGui(MyGeometry, MouseEvent.GetScreenSpacePosition()), MouseEvent);
}

FReply SCommonImGuiWidget::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& FocusEvent)
{
	Super::OnFocusReceived(MyGeometry, FocusEvent);

	IMGUI_WIDGET_LOG(VeryVerbose, TEXT("ImGui Widget %d - Focus Received."), ContextIndex);

	bForegroundWindow = GetIsForegroundWindow();
...
void SCommonImGuiWidget::OnFocusLost(const FFocusEvent& FocusEvent)
{
...
void SCommonImGuiWidget::OnMouseEnter(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
void SCommonImGuiWidget::OnMouseLeave(const FPointerEvent& MouseEvent)
{
...
FReply SCommonImGuiWidget::OnTouchStarted(const FGeometry& MyGeometry, const FPointerEvent& TouchEvent)
{
	return InputHandler->OnTouchStarted(TransformScreenPointToImGui(MyGeometry, TouchEvent.GetScreenSpacePosition()), TouchEvent);
}

FReply SCommonImGuiWidget::OnTouchMoved(const FGeometry& MyGeometry, const FPointerEvent& TouchEvent)
{
	return InputHandler->OnTouchMoved(TransformScreenPointToImGui(MyGeometry, TouchEvent.GetScreenSpacePosition()), TouchEvent);
}

FReply SCommonImGuiWidget::OnTouchEnded(const FGeometry& MyGeometry, const FPointerEvent& TouchEvent)
{
	UpdateVisibility();
	return InputHandler->OnTouchEnded(TransformScreenPointToImGui(MyGeometry, TouchEvent.GetScreenSpacePosition()), TouchEvent);
}

void SCommonImGuiWidget::CreateInputHandler(const FSoftClassPath& HandlerClassReference)
{
	ReleaseInputHandler();

	if(!Outer.IsValid())
	{
		Outer = GEngine->GetWorld();
#if WITH_EDITOR
		Outer = GEditor->EditorWorld;
#endif
		
	}
	if (!InputHandler.IsValid())
	{
		InputHandler = FImGuiInputHandlerFactory::NewHandler(HandlerClassReference, ModuleManager, Outer.Get(), ContextIndex);
	}
}

void SCommonImGuiWidget::ReleaseInputHandler()
{
	if (InputHandler.IsValid())
	{
...
void SCommonImGuiWidget::RegisterImGuiSettingsDelegates()
{
	auto& Settings = ModuleManager->GetSettings();

	if (!Settings.OnImGuiInputHandlerClassChanged.IsBoundToObject(this))
	{
		Settings.OnImGuiInputHandlerClassChanged.AddRaw(this, &SCommonImGuiWidget::CreateInputHandler);
	}
	if (!Settings.OnUseSoftwareCursorChanged.IsBoundToObject(this))
	{
		Settings.OnUseSoftwareCursorChanged.AddRaw(this, &SCommonImGuiWidget::SetHideMouseCursor);
	}
	if (!Settings.OnDPIScaleChangedDelegate.IsBoundToObject(this))
	{
		Settings.OnDPIScaleChangedDelegate.AddRaw(this, &SCommonImGuiWidget::SetDPIScale);
	}
	if (!Settings.OnCanvasSizeChangedDelegate.IsBoundToObject(this))
	{
		Settings.OnCanvasSizeChangedDelegate.AddRaw(this, &SCommonImGuiWidget::SetCanvasSizeInfo);
	}
}

void SCommonImGuiWidget::UnregisterImGuiSettingsDelegates()
{
	auto& Settings = ModuleManager->GetSettings();
...
void SCommonImGuiWidget::SetHideMouseCursor(bool bHide)
{
...
bool SCommonImGuiWidget::IsConsoleOpened() const
{
	return  false;//GameViewport->ViewportConsole && GameViewport->ViewportConsole->ConsoleState != NAME_None;
}

void SCommonImGuiWidget::UpdateVisibility()
{
...
void SCommonImGuiWidget::UpdateMouseCursor()
{
	if (!bHideMouseCursor)
	{
...
ULocalPlayer* SCommonImGuiWidget::GetLocalPlayer() const
{
	return nullptr; //TODO::重新实现

}

void SCommonImGuiWidget::TakeFocus()
{
...
void SCommonImGuiWidget::ReturnFocus()
{
	if (HasKeyboardFocus())
	{
...
void SCommonImGuiWidget::UpdateInputState()
{
	...
	{
		const auto& ViewportWidget = SharedThis(this);
...
void SCommonImGuiWidget::UpdateTransparentMouseInput(const FGeometry& AllottedGeometry)
{
...
void SCommonImGuiWidget::HandleWindowFocusLost()
{
	...
	{
		if (bForegroundWindow != GetIsForegroundWindow())
		{
...
void SCommonImGuiWidget::SetDPIScale(const FImGuiDPIScaleInfo& ScaleInfo)
{
...
void SCommonImGuiWidget::SetCanvasSizeInfo(const FImGuiCanvasSizeInfo& CanvasSizeInfo)
{
...
void SCommonImGuiWidget::UpdateCanvasSize(const FGeometry& AllottedGeometry)
{
	if (bUpdateCanvasSize)
	{
		if (auto* ContextProxy = ModuleManager->GetContextManager().GetContextProxy(ContextIndex))
		{
			...
			if (bAdaptiveCanvasSize )
			{
				...
				CanvasSize = ImGuiWidgetUtils::MaxVector(CanvasSize, ViewportSize);
			}
			else
			{
			...
			CanvasSize =ImGuiWidgetUtils::RoundVector(CanvasSize);
...
void SCommonImGuiWidget::UpdateCanvasControlMode(const FInputEvent& InputEvent)
{
...
void SCommonImGuiWidget::OnPostImGuiUpdate()
{
	ImGuiRenderTransform = ImGuiTransform;
	UpdateMouseCursor();
}

FVector2D SCommonImGuiWidget::TransformScreenPointToImGui(const FGeometry& MyGeometry, const FVector2D& Point) const
{
...
int32 SCommonImGuiWidget::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect,
	FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& WidgetStyle, bool bParentEnabled) const
{
	if (FImGuiContextProxy* ContextProxy = ModuleManager->GetContextManager().GetContextProxy(ContextIndex))
	{
		...
		const FSlateRenderTransform ImGuiToScreen = ImGuiWidgetUtils::RoundTranslation(ImGuiRenderTransform.Concatenate(WidgetToScreen));
...
FVector2D SCommonImGuiWidget::ComputeDesiredSize(float Scale) const
{
...
void SCommonImGuiWidget::OnDebugDraw()
{
